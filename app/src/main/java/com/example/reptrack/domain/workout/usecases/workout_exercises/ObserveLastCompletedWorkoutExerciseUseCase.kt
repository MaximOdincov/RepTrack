...
class ObserveLastCompletedWorkoutExerciseUseCase(
    private val workoutExerciseRepository: WorkoutExerciseRepository,
    private val errorHandler: ErrorHandler
) {
    suspend operator fun invoke(exerciseId: String): Flow<WorkoutExercise?> {
        return workoutExerciseRepository.observeLastCompletedExercise(exerciseId)
            .catchAndLog(
                errorHandler = errorHandler,
                context = ErrorContext(
                    screen = "WorkoutSession",
                    action = "ObserveLastCompletedWorkoutExercise",
                    entityId = exerciseId
                )
            )
    }
